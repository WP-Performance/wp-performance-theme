...
use function WP_Performance\Inc\Core\Helpers\get_manifest;

test('get token from manifest', function () {
    $config = get_manifest('/inc/core/tests/manifest.json');
    $files = get_object_vars($config);

    $token = WP_Performance\Inc\Core\Helpers\get_token_name($files['main-legacy.js']->file);
    expect($token)->toBe('fe2da1bc');

    $token = WP_Performance\Inc\Core\Helpers\get_token_name($files['main.js']->file);
...
test('keep only entries', function () {
    ...
    $cleaned = WP_Performance\Inc\Core\Helpers\keepEntries($files);
...
test('get legacy and polyfill', function () {
    ...
    $results = WP_Performance\Inc\Core\Helpers\moveLegacyAndPolyfill($files);
...
test('order manifest', function () {
    ...
    $results = WP_Performance\Inc\Core\Helpers\order_manifest($files);
